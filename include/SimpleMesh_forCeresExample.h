...
struct Triangle_ceres {
	unsigned int idx0;
	unsigned int idx1;
	unsigned int idx2;

	Triangle_ceres() : idx0{ 0 }, idx1{ 0 }, idx2{ 0 } {}

	Triangle_ceres(unsigned int _idx0, unsigned int _idx1, unsigned int _idx2) :
		idx0(_idx0), idx1(_idx1), idx2(_idx2) {}
};


class SimpleMesh_ceres {
public:
	SimpleMesh_ceres() {}
	 ...
	 */
	SimpleMesh_ceres(VirtualSensor& sensor, const Matrix4f& cameraPose, float edgeThreshold = 0.01f) {
	...
	unsigned int addVertex(Vertex_ceres& vertex) {
	...
	unsigned int addFace(unsigned int idx0, unsigned int idx1, unsigned int idx2) {
		...
		Triangle_ceres triangle(idx0, idx1, idx2);
		m_triangles.push_back(triangle);
		return fId;
	}

	std::vector<Vertex_ceres>& getVertices() {
		return m_vertices;
	}

	const std::vector<Vertex_ceres>& getVertices() const {
		return m_vertices;
	}

	std::vector<Triangle_ceres>& getTriangles() {
		return m_triangles;
	}

	const std::vector<Triangle_ceres>& getTriangles() const {
		return m_triangles;
	}

	void transform(const Matrix4f& transformation) {
		for (Vertex_ceres& v : m_vertices) {
	 ...
	 */
	static SimpleMesh_ceres joinMeshes(const SimpleMesh_ceres& mesh1, const SimpleMesh_ceres& mesh2, Matrix4f pose1to2 = Matrix4f::Identity()) {
		SimpleMesh_ceres joinedMesh;
		...
		for (int i = 0; i < nTriangles2; ++i) {
			...
			Triangle_ceres t{ t2.idx0 + nVertices1, t2.idx1 + nVertices1, t2.idx2 + nVertices1 };
	 ...
	 */
	static SimpleMesh_ceres sphere(Vector3f center, float scale = 1.f, Vector4uc color = { 0, 0, 255, 255 }) {
		SimpleMesh_ceres mesh;
	 ...
	 */
	static SimpleMesh_ceres camera(const Matrix4f& cameraPose, float scale = 1.f, Vector4uc color = { 255, 0, 0, 255 }) {
		SimpleMesh_ceres mesh;
	 ...
	 */
	static SimpleMesh_ceres cylinder(const Vector3f& p0, const Vector3f& p1, float radius, unsigned stacks, unsigned slices, const Vector4uc color = Vector4uc{ 0, 0, 255, 255 }) {
		SimpleMesh_ceres mesh;
...
private:
	std::vector<Vertex_ceres> m_vertices;
	std::vector<Triangle_ceres> m_triangles;
