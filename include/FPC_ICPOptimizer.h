 ...
 */
template <typename T>
static inline void FPC_fillVector(const Vector3f& input, T* output) {
	output[0] = T(input[0]);
	output[1] = T(input[1]);
	output[2] = T(input[2]);
}
 ...
 */
template <typename T>
class FPC_PoseIncrement {
public:
	explicit FPC_PoseIncrement(T* const array) : m_array{ array } { }
	 ...
	 */
	static Matrix4f convertToMatrix(const FPC_PoseIncrement<double>& poseIncrement) {
		// pose[0,1,2] is angle-axis rotation.
		// pose[3,4,5] is translation.
 ...
 */
class FPC_PointToPointConstraint {
public:
	FPC_PointToPointConstraint(const Vector3f& sourcePoint, const Vector3f& targetPoint, const float weight) :
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		//Source point p_s (red bunny point clouds)
			// m_sourcePoint(Vector3f) in FPC_PointToPointConstraint (one vertex)
		
		//Target point p_t (green bunny point clouds)
			// m_targetPoint(Vector3f) in FPC_PointToPointConstraint (one vertex)
		
		//weight (additional parameters to be optimized)
			// m_weight (float) in FPC_PointToPointConstraint (one vertex)
		
		// poseIncrement.m_array(private) = pose
		FPC_PoseIncrement<T> poseIncrement = FPC_PoseIncrement<T>(const_cast<T* const>(pose));
		...
		*/
		...
		T source_point[3];
		FPC_fillVector(m_sourcePoint, source_point);
		...
		T targetPoint[3];
		FPC_fillVector(m_targetPoint, targetPoint);
	...
	static ceres::CostFunction* create(const Vector3f& sourcePoint, const Vector3f& targetPoint, const float weight) {
		return new ceres::AutoDiffCostFunction<FPC_PointToPointConstraint, 3, 6>(
			new FPC_PointToPointConstraint(sourcePoint, targetPoint, weight)
		);
	}
...
class FPC_PointToPlaneConstraint {
public:
	FPC_PointToPlaneConstraint(const Vector3f& sourcePoint, const Vector3f& targetPoint, const Vector3f& targetNormal, const float weight) :
		m_sourcePoint{ sourcePoint },
		m_targetPoint{ targetPoint },
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		//Target Normal (normal of vertex in green bunny point clouds)
			// m_targetNormal(Vector3f) in FPC_PointToPlaneConstraint (one vertex)
		...
		*/
		...
		T sourcePoint[3];
		FPC_fillVector(m_sourcePoint, sourcePoint);
		...
		T targetNormal[3];
		FPC_fillVector(m_targetNormal, targetNormal);
	...
	static ceres::CostFunction* create(const Vector3f& sourcePoint, const Vector3f& targetPoint, const Vector3f& targetNormal, const float weight) {
		return new ceres::AutoDiffCostFunction<FPC_PointToPlaneConstraint, 1, 6>(
			new FPC_PointToPlaneConstraint(sourcePoint, targetPoint, targetNormal, weight)
		);
	}
...
public:
	...
	virtual Matrix4f estimatePose(const FacePointCloud& source, const FacePointCloud& target, Matrix4f initialPose = Matrix4f::Identity()) = 0;
 ...
 */
class FPC_CeresICPOptimizer : public FPC_ICPOptimizer {
public:
	FPC_CeresICPOptimizer() {}

	virtual Matrix4f estimatePose(const FacePointCloud& source, const FacePointCloud& target, Matrix4f initialPose = Matrix4f::Identity()) override {
		...
		auto poseIncrement = FPC_PoseIncrement<double>(incrementArray);
		...
		for (int i = 0; i < m_nIterations; ++i) {
			...
			Matrix4f matrix = FPC_PoseIncrement<double>::convertToMatrix(poseIncrement);
			estimatedPose = FPC_PoseIncrement<double>::convertToMatrix(poseIncrement) * estimatedPose;
			poseIncrement.setZero();
		...
		for (unsigned i = 0; i < nPoints; ++i) {
			...
			*/ 
			...
			if (match.idx >= 0) {
				...
				// FPC_PointToPointConstraint ptpc = {sourcePoint, targetPoint, weight}; 
				ceres::CostFunction * cost_function = FPC_PointToPointConstraint::create(sourcePoint, targetPoint, weight);
				...
				if (m_bUsePointToPlaneConstraints) {
					...
					// FPC_PointToPlaneConstraint n_ptpc = {sourcePoint, targetPoint, targetNormal, weight}; 
					ceres::CostFunction * cost_function = FPC_PointToPlaneConstraint::create(sourcePoint, targetPoint, targetNormal, weight);
					problem.AddResidualBlock(cost_function, nullptr, poseIncrement.getData());
 ...
 */
class FPC_LinearICPOptimizer : public FPC_ICPOptimizer {
public:
	FPC_LinearICPOptimizer(bool reg): regularizer(reg){}
